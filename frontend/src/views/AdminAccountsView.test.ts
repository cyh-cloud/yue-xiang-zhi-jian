...
import accountsViewSource from './AdminAccountsView.vue?raw'
...
function accountFixture(patch: Partial<AdminAccount> = {}): AdminAccount {
  return {
    id: 2,
    ...
    updated_at: '2026-09-20T14:05:00+08:00',
    ...patch
  }
}

const longNameAccount = accountFixture({
  id: 5,
  username: 'gov-policy-editor',
  name: '粤乡智匠平台政策新闻管理办公室负责人',
  role: 'government',
  is_enabled: false,
...
const adminAccount = accountFixture({
  id: 7,
  ...
  updated_at: '2026-09-20T11:12:00+08:00'
})

const defaultAccounts = [accountFixture(), longNameAccount, adminAccount]

let listAccounts: AdminAccount[] = defaultAccounts
let listFailure: Error | null = null

function accountsResponse(path: string): unknown {
  const pathname = path.split('?')[0] ?? path
  if (pathname.startsWith('/api/admin/accounts/')) {
    if (path.endsWith('/status')) {
      return { success: true, account: accountFixture({ is_enabled: false }) }
    }
    return {
      success: true,
      event_id: 'admin-password-reset:2:4',
...
apiFetchMock.mockImplementation(async (path: string) => {
  ...
  return accountsResponse(path)
})

async function mountView(
  accounts: AdminAccount[] = defaultAccounts,
...
describe('AdminAccountsView', () => {
  ...
  it('creates an enterprise account and resets its password', async () => {
    ...
    await wrapper.get('[data-test="account-name"]').setValue('粤乡农业科技有限公司')
    ...
    const createCall = apiFetchMock.mock.calls.find(
      call => call[0] === '/api/admin/accounts' && call[1]?.method === 'POST'
    )
    ...
    expect(wrapper.get('[data-test="account-create-message"]').text()).toContain(
      'enterprise-new'
    )

    await wrapper.get('[data-test="account-reset"]').trigger('click')
    await wrapper.get('[data-test="account-reset-confirm"]').trigger('click')
    await flushPromises()

    expect(apiFetch).toHaveBeenCalledWith(
      '/api/admin/accounts/2/password-reset',
      { method: 'POST' }
    )
    expect(wrapper.get('[data-test="account-action-message"]').text()).toContain(
      'enterprise-new'
    )
  })

  it('renders the required account columns and filters by role and keyword', async () => {
    const { wrapper } = await mountView()
    const headers = wrapper.findAll('[data-test="account-table"] thead th')
    ...
    const firstRow = wrapper.get('[data-test="account-row"]')
    expect(firstRow.text()).toContain('#2')
    expect(firstRow.text()).toContain('enterprise-new')
    expect(firstRow.text()).toContain('粤乡农业科技有限公司')
    expect(firstRow.text()).toContain('企业账户')
    expect(firstRow.text()).toContain('已启用')
    expect(firstRow.text()).toContain('2026-09-18 09:30')
    expect(firstRow.text()).toContain('2026-09-20 14:05')

    expect(wrapper.findAll('[data-test="account-row"]')).toHaveLength(3)

    await wrapper.get('[data-test="account-filter-government"]').trigger('click')
    await flushPromises()

    const roleCall = apiFetchMock.mock.calls.find(
      call => call[0] === '/api/admin/accounts?role=government'
    )
    expect(roleCall).toBeTruthy()

    await wrapper.get('[data-test="account-filter-all"]').trigger('click')
    await wrapper.get('[data-test="account-search"]').setValue('粤乡')
    await settleKeywordSearch()

    expect(
      apiFetchMock.mock.calls.some(
        call => call[0] === '/api/admin/accounts?keyword=%E7%B2%A4%E4%B9%A1'
      )
    ).toBe(true)
  })

  it('opens the detail panel for the selected account', async () => {
    const { wrapper } = await mountView()

    expect(wrapper.find('[data-test="account-detail-panel"]').exists()).toBe(false)

    const rows = wrapper.findAll('[data-test="account-row"]')
    await rows[1].get('[data-test="account-detail"]').trigger('click')

    const panel = wrapper.get('[data-test="account-detail-panel"]')
    expect(panel.text()).toContain('粤乡智匠平台政策新闻管理办公室负责人')
    expect(panel.text()).toContain('政府账户')
    expect(panel.text()).toContain('已禁用')
    expect(panel.text()).toContain('2026-08-01 08:00')
  })

  it('asks for confirmation before changing the enabled state and calls the status endpoint', async () => {
    const { wrapper } = await mountView()

    await wrapper.get('[data-test="account-toggle"]').trigger('click')
    expect(wrapper.find('[data-test="account-toggle-dialog"]').exists()).toBe(true)
    expect(apiFetch).not.toHaveBeenCalledWith(
      '/api/admin/accounts/2/status',
      expect.anything()
    )

    await wrapper.get('[data-test="account-toggle-cancel"]').trigger('click')
    expect(wrapper.find('[data-test="account-toggle-dialog"]').exists()).toBe(false)

    await wrapper.get('[data-test="account-toggle"]').trigger('click')
    await wrapper.get('[data-test="account-toggle-confirm"]').trigger('click')
    await flushPromises()

    expect(apiFetch).toHaveBeenCalledWith('/api/admin/accounts/2/status', {
      method: 'POST',
  ...
  it('states the reset delivery channel and never renders a password value', async () => {
    const { wrapper } = await mountView()
    ...
    await wrapper.get('[data-test="account-reset"]').trigger('click')
    const dialog = wrapper.get('[data-test="account-reset-dialog"]')

    expect(dialog.text()).toContain('02')
    expect(dialog.text()).toContain('线下')
    expect(dialog.text()).toContain('不展示密码内容')
    expect(dialog.find('input').exists()).toBe(false)
    expect(dialog.text()).not.toContain('password8')
    expect(dialog.html()).not.toContain('password8')
    expect(wrapper.html()).not.toContain('password8')
    expect(wrapper.text()).not.toContain('password8')
  })

  it('exposes no delete control anywhere on the screen', async () => {
    const { wrapper } = await mountView()
    ...
    expect(wrapper.find('[data-test="account-reset-dialog"]').exists()).toBe(true)
  })

  it('renders loading, error, and empty states for the account list', async () => {
    const failed = await mountView(
      [],
      new ApiError('账户列表加载失败', 500)
    )
    expect(failed.wrapper.get('[data-test="account-error"]').text()).toContain(
      '账户列表加载失败'
    )
    await failed.wrapper.get('[data-test="account-retry"]').trigger('click')
    await flushPromises()
    expect(
      apiFetchMock.mock.calls.filter(call => call[0] === '/api/admin/accounts')
        .length
    ).toBeGreaterThan(1)

    listFailure = null
    const empty = await mountView([])
    expect(empty.wrapper.get('[data-test="account-empty"]').text()).toContain(
      '暂无符合条件的账户'
    )
  })

  it('hides the accounts navigation entry and route from ordinary admins', async () => {
    ...
    const ordinary = mount(AdminConsoleNav, {
      props: { role: 'admin' },
      global: { plugins: [guardRouter], stubs: { RouterLink: RouterLinkStub } }
    })
    expect(ordinary.find('[data-test="admin-nav-accounts"]').exists()).toBe(false)

    const superAdmin = mount(AdminConsoleNav, {
      props: { role: 'super_admin' },
      global: { plugins: [guardRouter], stubs: { RouterLink: RouterLinkStub } }
    })
    expect(superAdmin.find('[data-test="admin-nav-accounts"]').exists()).toBe(true)

    const record = router
      .getRoutes()
      .find(route => route.path === '/admin/accounts')
    expect(record?.name).toBe('admin-accounts')
...
describe('AdminAccountsView geometry', () => {
  it('keeps the screen root unclipped so the geometry assertion has teeth', () => {
    const rootRule = cssRule(accountsViewSource, '.admin-accounts')
  ...
  it('collapses fixed-format regions with responsive constraints', () => {
    expect(cssRule(accountsViewSource, '.accounts-roles')).toContain(
      'grid-template-columns: repeat(5, minmax(0, 1fr))'
    )
    expect(
      cssRule(accountsViewSource, '.accounts-roles', 560)
    ).toContain('grid-template-columns: repeat(2, minmax(0, 1fr))')
    expect(cssRule(accountsViewSource, '.accounts-create__form')).toContain(
      'grid-template-columns: repeat(5, minmax(0, 1fr))'
    )
    expect(
      cssRule(accountsViewSource, '.accounts-create__form', 560)
    ).toContain('grid-template-columns: minmax(0, 1fr)')
    expect(cssRule(accountsViewSource, '.accounts-table')).toContain(
      'table-layout: fixed'
    )
    expect(cssRule(accountsViewSource, '.accounts-table', 1120)).toContain(
      'display: block'
    )
  })
  ...
  it('wraps long Chinese text without mid-word breaks', () => {
    for (const selector of [
      ...
      const rule = cssRule(accountsViewSource, selector)
  ...
  for (const width of [320, 375, 1280] as const) {
    it(`has no horizontal overflow at ${width}px with long Chinese content`, async () => {
      const { wrapper } = await mountView()
      const root = wrapper.get('[data-test="admin-accounts"]').element as HTMLElement
      installGeometry(root, accountsViewSource, width)
  ...
  it('detects overflowing content so the geometry assertion is not vacuous', () => {
    const host = mount({
      template:
        '<div class="probe"><span class="probe-run">粤乡智匠账户管理后台长文本探针</span></div>'
    })
