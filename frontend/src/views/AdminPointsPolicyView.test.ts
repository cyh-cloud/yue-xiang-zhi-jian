...
import type { AdminPointsPolicy, AdminPointsPolicyPayload } from '@/api/types'
import AdminConsoleNav from '@/components/AdminConsoleNav.vue'
import router from '@/router'
import { useAuthStore } from '@/stores/auth'
import { authGuard } from '@/router/roleRoutes'

import AdminPointsPolicyView from './AdminPointsPolicyView.vue'
import policyViewSource from './AdminPointsPolicyView.vue?raw'
...
function policyFixture(): AdminPointsPolicy {
  return {
    version: 3,
    rule_version: 'v3',
...
function installDefaultFetch() {
  apiFetchMock.mockImplementation(
    async (path: string, options?: RequestInit) => {
      if (path === '/api/admin/points-policy' && options?.method === 'PUT') {
        if (nextPutStatus === 'conflict') {
          // Another admin already committed version 4 while the form was open.
          currentPolicy = {
            ...currentPolicy,
            version: currentPolicy.version + 1,
            seconds_per_point: 999,
            updated_by: 42,
        ...
        const body = JSON.parse(
          String(options?.body)
        ) as AdminPointsPolicyPayload
        const nextVersion = currentPolicy.version + 1
        currentPolicy = {
          version: nextVersion,
          rule_version: `v${nextVersion}`,
          seconds_per_point: body.seconds_per_point,
          training_weights: { ...body.training_weights },
      ...
      if (path === '/api/admin/points-policy') {
        return { success: true, policy: currentPolicy }
      }
      return { success: true }
    }
...
function inputValue(wrapper: ReturnType<typeof mount>, testId: string): string {
  return (wrapper.get(`[data-test="${testId}"]`).element as HTMLInputElement)
    .value
}

function putBodies(): AdminPointsPolicyPayload[] {
...
describe('AdminPointsPolicyView', () => {
  ...
  it('submits all five training weight keys and the expiry mode', async () => {
    ...
    await wrapper.get('[data-test="weight-simulation"]').setValue('3')
    await wrapper.get('[data-test="weight-copy-training"]').setValue('4')
    await wrapper.get('[data-test="weight-customer-service"]').setValue('6')
    await wrapper.get('[data-test="daily-limit"]').setValue('80')
    await wrapper.get('[data-test="expiry-natural-year"]').setValue('natural_year')
    ...
    expect(body.training_weights).toEqual(
      expect.objectContaining({
        default: expect.any(Number),
        live_script: expect.any(Number),
        simulation: expect.any(Number),
        copy_training: expect.any(Number),
        customer_service: expect.any(Number)
      })
    )
    expect(wrapper.get('[data-test="points-policy-message"]').text()).toContain(
      '已保存'
    )
  })

  it('seeds the form from the server policy instead of hard-coded defaults', async () => {
    const { wrapper } = await mountView()

    expect(inputValue(wrapper, 'seconds-per-point')).toBe('600')
    expect(inputValue(wrapper, 'daily-limit')).toBe('60')
    expect(inputValue(wrapper, 'weight-default')).toBe('10')
    expect(inputValue(wrapper, 'weight-customer-service')).toBe('10')
    expect(wrapper.get('[data-test="policy-rule-version"]').text()).toBe('v3')
    expect(wrapper.get('[data-test="policy-updated-by"]').text()).toBe(
      '系统初始化'
    )
    expect(wrapper.get('[data-test="policy-updated-at"]').text()).toBe(
      '2026-09-21 10:00'
    )
  })
  ...
  it('keeps the server values and bumps the version after a 409 so a retry succeeds', async () => {
    nextPutStatus = 'conflict'
    const { wrapper } = await mountView()
    ...
    // The failed PUT reloaded the authoritative row (version 4, seconds 999),
    // so the form re-seeded to those values and the conflict banner appeared.
    expect(inputValue(wrapper, 'seconds-per-point')).toBe('999')
    ...
    const retry = bodies[bodies.length - 1]
    expect(retry.expected_version).toBe(4)
    expect(retry.seconds_per_point).toBe(999)
    expect(wrapper.get('[data-test="points-policy-message"]').text()).toContain(
      '已保存'
    )
  ...
  it('surfaces a load error and reloads the policy on retry', async () => {
    ...
    apiFetchMock.mockImplementation(
      async (path: string, options?: RequestInit) => {
        if (path === '/api/admin/points-policy' && options?.method !== 'PUT') {
          if (failLoad) {
            throw new ApiError('积分规则加载失败', 500)
          }
          return { success: true, policy: currentPolicy }
        }
        return { success: true }
    ...
    failLoad = false
    await wrapper.get('[data-test="points-policy-retry"]').trigger('click')
    await flushPromises()

    expect(wrapper.find('[data-test="points-policy-error"]').exists()).toBe(false)
    expect(inputValue(wrapper, 'seconds-per-point')).toBe('600')
  })

  it('hides the points-policy navigation entry and route from ordinary admins', async () => {
    ...
    expect(ordinary.find('[data-test="admin-nav-points-policy"]').exists()).toBe(
      false
    )
    ...
    expect(superAdmin.find('[data-test="admin-nav-points-policy"]').exists()).toBe(
      true
    )

    const record = router
      .getRoutes()
      .find(route => route.path === '/admin/points-policy')
    expect(record?.name).toBe('admin-points-policy')
...
describe('AdminPointsPolicyView geometry', () => {
  // Blind spot: the confirm-free form has no fixed-overlay dialog, so nothing
  // is deliberately excluded from the in-flow measurement below.
  it('keeps the screen root unclipped so the geometry assertion has teeth', () => {
    const rootRule = cssRule(policyViewSource, '.admin-points-policy')
  ...
  it('collapses fixed-format regions with responsive constraints', () => {
    expect(cssRule(policyViewSource, '.pp-scalars')).toContain(
      'grid-template-columns: repeat(2, minmax(0, 1fr))'
    )
    expect(cssRule(policyViewSource, '.pp-scalars', 560)).toContain(
      'grid-template-columns: minmax(0, 1fr)'
    )
    expect(cssRule(policyViewSource, '.pp-form__grid')).toContain(
      'grid-template-columns: repeat(5, minmax(0, 1fr))'
    )
    expect(cssRule(policyViewSource, '.pp-form__grid', 900)).toContain(
      'grid-template-columns: repeat(2, minmax(0, 1fr))'
    )
    expect(cssRule(policyViewSource, '.pp-form__grid', 560)).toContain(
      'grid-template-columns: minmax(0, 1fr)'
    )
    expect(cssRule(policyViewSource, '.pp-segmented')).toContain(
      'grid-template-columns: repeat(2, minmax(0, 1fr))'
    )
    expect(cssRule(policyViewSource, '.pp-segmented', 560)).toContain(
      'grid-template-columns: minmax(0, 1fr)'
    )
  })
  ...
  it('wraps long Chinese text without mid-word breaks', () => {
    for (const selector of [
      ...
      const rule = cssRule(policyViewSource, selector)
  ...
  for (const width of [320, 375, 1280] as const) {
    it(`has no horizontal overflow at ${width}px with long Chinese content`, async () => {
      const { wrapper } = await mountView()
      const root = wrapper.get('[data-test="admin-points-policy"]')
        .element as HTMLElement
      installGeometry(root, policyViewSource, width)
  ...
  it('detects overflowing content so the geometry assertion is not vacuous', () => {
    const host = mount({
      template:
        '<div class="probe"><span class="probe-run">粤乡智匠积分规则后台长文本探针</span></div>'
    })
