...
describe('MessageCenterView session expiry', () => {
  beforeEach(() => {
  ...
  it('redirects an expired message request to login with the current route', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          success: false,
          message: '未登录或会话已过期',
          redirect: '/login?redirect=%2Fapi%2Fmessages%2Fsummary'
        })
      })
    )
