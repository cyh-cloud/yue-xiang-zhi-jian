...
import announcementsViewSource from './AdminAnnouncementsView.vue?raw'
...
function announcementFixture(
  ...
  return {
    announcement_id: 'announcement-9f1c2b7a',
    title: '非遗技艺培训批次调整通知',
    body: '本周六凌晨进行系统维护，维护期间暂停兑换与履约操作。',
    target_roles: ['student', 'teacher'],
    status: 'draft',
    event_id: 'announcement-9f1c2b7a:publish',
    created_by: 1,
    created_at: '2026-09-20T10:15:00+08:00',
    published_at: null,
    ...patch
  }
}

// Long Chinese content keeps the geometry probe meaningful at 320/375/1280.
function longAnnouncementFixture(): AdminAnnouncement {
  return announcementFixture({
    announcement_id: 'announcement-long-chinese-probe',
    title: '粤乡智匠平台关于非遗技艺培训批次与奖品兑换规则的补充说明公告',
    body: '本周六凌晨二点至四点进行系统维护，维护期间暂停课程视频审核、奖品兑换与履约发放操作，请各位管理员提前安排工作进度，避免在维护窗口内提交审核意见。',
    target_roles: [
      'student',
      'teacher',
      'enterprise',
...
function publishedAnnouncementFixture(): AdminAnnouncement {
  return announcementFixture({
    announcement_id: 'announcement-4d8e0c11',
    title: '九月积分发放口径说明',
    body: '积分发放总量按奖励流水口径统计，禁用账户计入总数。',
    target_roles: ['student'],
    status: 'published',
    event_id: 'announcement-4d8e0c11:publish',
...
function resetState(): void {
  ...
  deliveredPublishes = 0
  deliveryFailuresRemaining = 0
  createFailure = null
}

function publishResponse(announcementId: string): unknown {
  ...
  if (publishedIds.has(announcementId)) {
    // The second publish is a no-op: no second outbox row, no second
    // user-visible notification.
    return {
      success: true,
      changed: false,
      announcement: target,
      delivery: { delivered: 0, recipient_count: 0, failed: 0 }
    }
  }
  publishedIds.add(announcementId)
  target.status = 'published'
  target.published_at = '2026-09-20T12:00:00+08:00'
  if (deliveryFailuresRemaining > 0) {
    ...
    return {
      ...
      changed: true,
      announcement: target,
      delivery: { delivered: 0, recipient_count: 3, failed: 1 }
    }
  }
  ...
  return {
    ...
    changed: true,
    announcement: target,
    delivery: { delivered: 1, recipient_count: 3, failed: 0 }
  }
}
  ...
  if (pathname === '/api/admin/announcements') {
    if (requestMethod(options) === 'POST') {
      ...
      const created = announcementFixture({
        announcement_id: 'announcement-created-0001',
        title: payload.title,
        body: payload.body,
        target_roles: payload.target_roles,
        status: 'draft',
        event_id: 'announcement-created-0001:publish',
    ...
    return { success: true, items: announcements, count: announcements.length }
  }
  if (pathname.endsWith('/publish')) {
    const id = decodeURIComponent(
      pathname.slice('/api/admin/announcements/'.length, -'/publish'.length)
    )
...
apiFetchMock.mockImplementation(async (path: string, options?: unknown) =>
  announcementsResponse(path, options)
)
...
apiFetchMock.mockImplementation(async (path: string, options?: unknown) =>
  ...
  it('registers the announcements route as super admin only', async () => {
    const record = router
      .getRoutes()
      .find(route => route.path === '/admin/announcements')

    expect(record?.name).toBe('admin-announcements')
    ...
    expect(await authGuard(target, auth)).toBe('/admin')
  ...
  it('denies an ordinary admin instead of showing an empty list', async () => {
    accessDenied = true
    const { wrapper, store } = await mountView('admin')

    expect(apiFetchMock).toHaveBeenCalledWith('/api/admin/announcements')
    expect(wrapper.find('[data-test="announcement-denied"]').exists()).toBe(true)
    expect(wrapper.find('[data-test="announcement-row"]').exists()).toBe(false)
    expect(wrapper.find('[data-test="announcement-empty"]').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('非遗技艺培训批次调整通知')
    expect(store.announcementAccessDenied).toBe(true)
    expect(store.announcements).toEqual([])
  })

  it('lists drafts and published announcements for a super admin', async () => {
    const { wrapper } = await mountView('super_admin')

    expect(wrapper.findAll('[data-test="announcement-row"]')).toHaveLength(3)
    expect(
      wrapper.find('[data-test="announcement-row"]').text()
    ).toContain('非遗技艺培训批次调整通知')
    expect(
      wrapper
        .find('[data-announcement-id="announcement-4d8e0c11"]')
        .get('[data-test="announcement-status"]')
        .text()
    ).toBe('已发布')
  })

  it('filters the list by announcement status', async () => {
    const { wrapper } = await mountView('super_admin')

    await wrapper.get('[data-test="announcement-filter-draft"]').trigger('click')
    await flushPromises()

    expect(wrapper.findAll('[data-test="announcement-row"]')).toHaveLength(2)
  })

  it('validates the create form client-side before calling the API', async () => {
    const { wrapper } = await mountView('super_admin')
    ...
    expect(wrapper.get('[data-test="announcement-form-error"]').text()).toContain(
      '公告标题不能为空'
    )
    expect(
    ...
    await wrapper.get('[data-test="announcement-title"]').setValue('系统维护通知')
    await wrapper.get('[data-test="announcement-submit"]').trigger('click')
    await flushPromises()
    ...
    await wrapper
      .get('[data-test="announcement-body"]')
      .setValue('本周六凌晨进行系统维护。')
    await wrapper.get('[data-test="announcement-role-student"]').setValue(false)
    await wrapper.get('[data-test="announcement-submit"]').trigger('click')
    await flushPromises()
  ...
  it('creates a draft announcement with the selected target roles', async () => {
    ...
    await wrapper
      .get('[data-test="announcement-body"]')
      .setValue('本周六凌晨进行系统维护。')
    await wrapper.get('[data-test="announcement-role-teacher"]').setValue(true)
    await wrapper.get('[data-test="announcement-submit"]').trigger('click')
    await flushPromises()
    ...
    expect(store.announcementFormError).toBe('')
  })

  it('surfaces the server 400 code and message from the create form', async () => {
    createFailure = new ApiError(
      'title 长度不能超过 60 个字符',
      400,
      {},
      undefined,
      'announcement_validation_failed'
    )
    const { wrapper } = await mountView('super_admin')
    await wrapper.get('[data-test="announcement-create"]').trigger('click')
    ...
    expect(wrapper.get('[data-test="announcement-server-error"]').text()).toContain(
      'title 长度不能超过 60 个字符'
    )
    expect(
      wrapper.get('[data-test="announcement-server-error-code"]').text()
    ).toBe('announcement_validation_failed')
  })

  it('publishes a draft only after the confirmation dialog', async () => {
    const { wrapper } = await mountView('super_admin')

    await wrapper.get('[data-test="announcement-publish"]').trigger('click')
    expect(
      wrapper.find('[data-test="announcement-publish-dialog"]').exists()
    ).toBe(true)

    await wrapper
      .get('[data-test="announcement-publish-confirm"]')
      .trigger('click')
    await flushPromises()

    expect(apiFetchMock).toHaveBeenCalledWith(
      '/api/admin/announcements/announcement-9f1c2b7a/publish',
      expect.objectContaining({ method: 'POST' })
    )
    expect(wrapper.find('[data-test="announcement-publish-dialog"]').exists()).toBe(
      false
    )
    expect(
      wrapper.get('[data-test="announcement-publish-outcome"]').text()
  ...
  it('notifies once when the same announcement is published twice', async () => {
    ...
    // The second publish is issued straight through the store because the
    // first one already moved the row out of the draft filter.
    await store.publishAnnouncement('announcement-9f1c2b7a')
    await flushPromises()
    await store.publishAnnouncement('announcement-9f1c2b7a')
    await flushPromises()

    expect(deliveredPublishes).toBe(1)
    expect(store.announcementPublishResult?.changed).toBe(false)
    expect(
      wrapper.get('[data-test="announcement-publish-outcome"]').text()
  ...
  it('shows loading, empty and error states for the announcement list', async () => {
    apiFetchMock.mockImplementation(() => new Promise(() => {}))
    const loading = await mountView('super_admin')
    expect(
      loading.wrapper.find('[data-test="announcement-loading"]').exists()
    ).toBe(true)
...
apiFetchMock.mockImplementation(async (path: string, options?: unknown) =>
    ...
    expect(
      empty.wrapper.find('[data-test="announcement-empty"]').exists()
    ).toBe(true)
    empty.wrapper.unmount()

    listFailure = new ApiError('公告列表加载失败', 500)
    const failed = await mountView('super_admin')
    expect(
      failed.wrapper.get('[data-test="announcement-error"]').text()
    ).toContain('公告列表加载失败')
  })
})

describe('AdminAnnouncementsView geometry', () => {
  beforeEach(() => {
    vi.clearAllMocks()
apiFetchMock.mockImplementation(async (path: string, options?: unknown) =>
      announcementsResponse(path, options)
  ...
  it('keeps the screen root unclipped so the geometry assertion has teeth', () => {
    const rootRule = cssRule(announcementsViewSource, '.admin-announcements')
  ...
  it('collapses fixed-format regions with responsive constraints', () => {
    expect(cssRule(announcementsViewSource, '.announcements-header')).toContain(
      'grid-template-columns: minmax(0, 1fr) minmax(0, 0.62fr)'
    )
    expect(
      cssRule(announcementsViewSource, '.announcements-header', 900)
    ).toContain('grid-template-columns: minmax(0, 1fr)')
    expect(cssRule(announcementsViewSource, '.announcements-toolbar')).toContain(
      'grid-template-columns: minmax(0, 1fr) 52px'
    )
    expect(cssRule(announcementsViewSource, '.announcements-filter')).toContain(
      'grid-template-columns: repeat(3, minmax(0, 1fr))'
    )
    expect(
      cssRule(announcementsViewSource, '.announcements-create__form')
    ).toContain('grid-template-columns: repeat(2, minmax(0, 1fr))')
    expect(
      cssRule(announcementsViewSource, '.announcements-create__form', 560)
    ).toContain('grid-template-columns: minmax(0, 1fr)')
    expect(cssRule(announcementsViewSource, '.announcements-roles')).toContain(
      'grid-template-columns: repeat(3, minmax(0, 1fr))'
    )
    expect(
      cssRule(announcementsViewSource, '.announcements-roles', 560)
    ).toContain('grid-template-columns: repeat(2, minmax(0, 1fr))')
    expect(cssRule(announcementsViewSource, '.announcements-item__head')).toContain(
      'grid-template-columns: minmax(0, 1fr) 76px'
    )
    expect(
      cssRule(announcementsViewSource, '.announcements-item__meta')
    ).toContain('grid-template-columns: repeat(3, minmax(0, 1fr))')
    expect(
      cssRule(announcementsViewSource, '.announcements-item__meta', 560)
    ).toContain('grid-template-columns: minmax(0, 1fr)')
  })
  ...
  it('wraps long Chinese text without mid-word breaks', () => {
    for (const selector of [
      ...
      const rule = cssRule(announcementsViewSource, selector)
  ...
  for (const width of [320, 375, 1280] as const) {
    it(`has no horizontal overflow at ${width}px with long Chinese content`, async () => {
      const { wrapper } = await mountView('super_admin')
      expect(wrapper.text()).toContain('补充说明公告')
      const root = wrapper.get('[data-test="admin-announcements"]')
        .element as HTMLElement
      installGeometry(root, announcementsViewSource, width)
  ...
  it('detects overflowing content so the geometry assertion is not vacuous', () => {
    const host = mount({
      template:
        '<div class="probe"><span class="probe-run">粤乡智匠系统公告长文本探针</span></div>'
    })
